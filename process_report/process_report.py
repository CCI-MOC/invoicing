...
def main():
    ...
    merged_dataframe = merge_csv(config.CSV_INVOICE_FILEPATH_LIST)

    logger.info("Invoice date: " + str(invoice_month))

    timed_projects_list = config.get_nonbillable_timed_projects(
        config.NONBILLABLE_TIMED_PROECTS_FILEPATH, invoice_month
    )
...
def process_merged_dataframe(
    ...
    for processor in processors:
        processor(data=dataframe)
        processor.process(invoice_month=invoice_month, data=dataframe)
