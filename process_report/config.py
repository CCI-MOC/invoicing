...
@functools.lru_cache
def get_csv_invoice_filepaths() -> list[str]:
    if not CSV_INVOICE_FILEPATH_LIST:
        return fetch_s3_invoices(INVOICE_MONTH)


@functools.lru_cache
def get_prepay_debits_filepath() -> str:
    if not PREPAY_DEBITS_FILEPATH:
        return util.fetch_s3(PREPAY_DEBITS_S3_FILEPATH)


@functools.lru_cache
def get_old_pi_filepath() -> str:
    if not OLD_PI_FILEPATH:
        return util.fetch_s3(PI_S3_FILEPATH)


@functools.lru_cache
def get_alias_filepath() -> str:
    if not ALIAS_FILEPATH:
        return util.fetch_s3(ALIAS_S3_FILEPATH)


@functools.lru_cache
def get_nonbillable_pis() -> list[str]:
    with open(NONBILLABLE_PIS_FILEPATH) as file:
        return [line.rstrip() for line in file]


@functools.lru_cache
def get_nonbillable_projects() -> list[str]:
...
@functools.lru_cache
def get_nonbillable_timed_projects(timed_projects_file, invoice_date) -> list[str]:
...
@functools.lru_cache
def get_lenovo_su_charge_info(invoice_month, rates_info) -> dict[str, Decimal]:
    su_charge_info = {}
